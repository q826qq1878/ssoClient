...
public class ExcelRead {
     ...
     * @throws IOException  
     */  
    public static List<ArrayList<String>> readExcel(InputStream file) throws IOException {  
        if(file==null||ExcelUtil.EMPTY.equals(((MultipartFile) file).getOriginalFilename().trim())){  
            return null;  
        }else{  
            String postfix = ExcelUtil.getPostfix(((MultipartFile) file).getOriginalFilename());  
	...
	public static boolean deleteFile(File file) {  
	    if(file.isDirectory()) {  
	        ...
	        for(int i = 0; i < childrenFiles.length; i++) {  
	            boolean success = deleteFile(new File(file,childrenFiles[i]));  
	            if(!success) {  
	                return false;  
	            }  
	        }  
